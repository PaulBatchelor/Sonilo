...
static uint32_t div_init(uint32_t *mem, uint16_t ctx)
{
    int rc;
    uint16_t stk;
    uint32_t cmd;
...
static uint32_t div_render(uint32_t *mem, uint16_t ugen)
{
    uint32_t *ports;
    sonilo_port ins[2], out;
    int n;
...
int ugen_arith(sonilo *s)
{
    ...
    if (rc) return rc;
    rc = sonilo_ugen_register(s, "DIV", div_init, div_render);
    if (rc) return rc;
