...
void parse_memwrite(memwrite *mw, char c)
{
    ...
    if (iscmd(mw, c, "rd")) {
        ...
        sonilo_vm_rw_set(vm, mw->mem[mw->cursor]);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "ba")) {
         ...
         */
        uint32_t rw;
        rw = blocklist_pop(mw->mem, mw->cursor);
        sonilo_vm_rw_set(vm, rw);
        /* TODO SET err */
        if (rw == 0) mw->err = 1;
        mw->prev = 0;
    ...
    if (iscmd(mw, c, "bf")) {
        ...
        uint32_t rw;
        rw = sonilo_vm_rw_get(vm);
        mw->err = blocklist_push(mw->mem, mw->cursor, rw & 0x3ff);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "sa")) {
        ...
        uint32_t rw;
        rw = sonilo_vm_rw_get(vm);
        bitset_add(mw->mem, mw->cursor, rw);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "se")) {
        ...
        uint32_t rw;
        rw = sonilo_vm_rw_get(vm);
        rw = bitset_exists(mw->mem, mw->cursor, rw);
        sonilo_vm_rw_set(vm, rw);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "sr")) {
        ...
        uint32_t rw;
        rw = sonilo_vm_rw_get(vm);
        bitset_remove(mw->mem, mw->cursor, rw);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "al")) {
         ...
         * TODO: GET cursor
         */
        rw = barray_length(mw->mem, mw->cursor);
        sonilo_vm_rw_set(vm, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "aa")) {
         ...
         */
        rw = sonilo_vm_rw_get(vm);
        mw->err = barray_append(mw->mem, mw->cursor, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "ap")) {
        ...
        mw->err = barray_pop(mw->mem, mw->cursor, rw);
        mw->prev = 0;
        return;
    }
    ...
    if (iscmd(mw, c, "zp")) {
         ...
         * DONE: SET rw */
        rw = sonilo_vm_rw_get(vm);
        rw = zero_page_init(mw->mem, mw->cursor, rw);
        sonilo_vm_rw_set(vm, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "xr")) {
         ...
         * TODO: GET cursor
         * DONE: SET rw */
        rw = bits_get(mw->mem, (mw->cursor << 4) + off, sz);
        sonilo_vm_rw_set(vm, rw);
    ...
    if (iscmd(mw, c, "sn")) {
         ...
         * TODO: GET cursor
         * TODO: GET mem */
        sonilo_vm_rw_set(vm, bitset_len(mw->mem, mw->cursor));
        return;
    }
    ...
    if (iscmd(mw, c, "bx")) {
         ...
         * TODO: SET err
         * DONE: GET rw (ptr) */
        mw->err = instr_block(mw->mem, &mw->instr, mw->cursor, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "rx")) {
        ...
        if (mode == 0) {
             ...
             * TODO: GET cursor
             * DONE: SET rw */
            rw = rc_get_count(mw->mem, mw->cursor, rw);
            sonilo_vm_rw_set(vm, rw);
            return;
        } else if (mode == 2) {
            /* TODO: GET mem
             * TODO: GET cursor
             * DONE: GET rw */
            rw = rc_get_hold(mw->mem, mw->cursor, rw);
            sonilo_vm_rw_set(vm, rw);
            return;
        } else if (mode == 3) {
            /* TODO: GET mem
             * TODO: GET cursor */
            rw = rc_get_active(mw->mem, mw->cursor);
            sonilo_vm_rw_set(vm, rw);
            return;
        }
        return;
    ...
    if (iscmd(mw, c, "rf")) {
         ...
         * DONE: SET rw */
        rw = sonilo_vm_rw_get(vm);
        rw = rc_find(mw->mem, mw->cursor, rw);
        sonilo_vm_rw_set(vm, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "ru")) {
        ...
        if (mode) {
             ...
             * TODO: GET cursor
             * DONE: SET rw */
            rw = rc_decr(mw->mem, mw->cursor, rw);
        } else {
            /* increment */
            /* DONE: SET rw
             * TODO: GET mem
             * TODO: GET cursor
             * DONE: GET rw */
            rw = rc_incr(mw->mem, mw->cursor, rw);
        }
        sonilo_vm_rw_set(vm, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "rh")) {
        ...
        if (mode) {
             ...
             * TODO: GET cursor
             * DONE: GET rw */
            rc_unhold(mw->mem, mw->cursor, rw);
        } else {
            /* hold */
            /* TODO: GET mem
             * TODO: GET cursor
             * DONE: GET rw */
            rc_hold(mw->mem, mw->cursor, rw);
        }
        sonilo_vm_rw_set(vm, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "rg")) {
         ...
         * TODO: GET mem
         * TODO: GET cursor */
        sonilo_vm_rw_set(vm, rc_get(mw->mem, mw->cursor));
        return;
    }
    ...
    if (iscmd(mw, c, "po")) {
         ...
         * TODO: GET cursor
         * TODO: GET rw (ptr) */
        mw->err = pstack_pop(mw->mem, mw->cursor, rw);
        return;
    }
    ...
    if (iscmd(mw, c, "mx")) {
         ...
         * TODO: GET mem
         * TODO: GET cursor */
        sonilo_vm_rw_set(vm, mem_aux(mw->mem, mw->cursor));
    ...
    if (iscmd(mw, c, "ws")) {
        ...
        /* DONE: SET rw
         * TODO: GET mem */
        sonilo_vm_rw_set(vm, array_value(mw->mem, ws));
    }
